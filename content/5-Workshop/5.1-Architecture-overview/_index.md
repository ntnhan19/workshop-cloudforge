...
1. **Frontend & Authentication:**
   ...
   - Client API requests are routed through **API Gateway** to the backend service running on **AWS App Runner**.
...
3. **AI Processing Pipeline:**
   - Heavy computational tasks are executed on **Amazon ECS (AWS Fargate)** within Private Subnets (with Auto Scaling across Availability Zones A & B).
   - ECS securely pulls videos and saves keyframes to S3 via an **S3 Gateway Endpoint** (PrivateLink), ensuring traffic remains on the AWS global network and optimizing data transfer costs.
...
4. **Data Persistence & Semantic Search:**
   ...
   - During a search request, **App Runner** calls Bedrock to generate a Query Vector and accesses the RDS database via a **VPC Connector** to execute the Vector Search.
