...
2. Tạo nhóm quản trị viên (Admin):
   ...
   *Giao diện tạo nhóm CloudForge-Admins*
...
3. Tạo nhóm lập trình viên (Dev - cho các thành viên cần deploy nhưng không có quyền xóa các tài nguyên lõi):
   ...
   *Giao diện tạo nhóm CloudForge-Developers*
...
4. Vào **IAM > Users > Create user**, tạo tài khoản cho các thành viên (ví dụ: `cf-vy`, `cf-luan`) và đưa họ vào các group tương ứng.
   ...
   *Màn hình gán User vào các nhóm Admin/Dev*
...
3. Đặt tên là `ecsTaskExecutionRole` và bấm Create.
   ...
   *Màn hình đính kèm AmazonECSTaskExecutionRolePolicy*
...
*Giao diện tạo thành công ECS-Backend-TaskRole*
...
*Màn hình Summary của ECS-Worker-TaskRole sau khi tạo thành công*
...
*Danh sách các Roles trong IAM khi gõ tìm kiếm từ khóa "ECS" và "StepFunctions"*
...
*Màn hình Terminal hiển thị kết quả gọi lệnh get-caller-identity thành công*
...
*Trang Model Access cũ trong Amazon Bedrock, xác nhận các model đã được tự động cung cấp*