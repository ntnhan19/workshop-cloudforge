...
Trong phần này, chúng ta sẽ xây dựng một Virtual Private Cloud (VPC) bảo mật và có tính sẵn sàng cao để lưu trữ toàn bộ kiến trúc của hệ thống Smart Media Analytics. Thay vì sử dụng mạng mặc định, việc tự thiết kế một VPC giúp chúng ta kiểm soát hoàn toàn luồng giao thông mạng (traffic flow), cô lập các dịch vụ nhạy cảm và tối ưu hóa chi phí đường truyền.

Chúng ta sẽ triển khai một kiến trúc mạng đa tầng (multi-tier network) bao gồm:
...
Bằng cách sử dụng công cụ **VPC and more** tích hợp sẵn trên AWS Console, toàn bộ các thành phần này cùng với hệ thống định tuyến (Route Tables) sẽ được khởi tạo tự động một cách chuẩn xác, giúp bạn tiết kiệm thời gian đáng kể.
