...
- **API type:** Chọn `Public` (do ALB của chúng ta có thể gọi qua Public DNS hoặc bạn có thể chọn `Private` nếu VPC nội bộ).
...
2. **Cấu hình tham số Arguments cho RunTask (JSONata):**
   ...
   - Chuyển sang tab **Arguments & Output**, bạn sẽ thấy một khung soạn thảo JSON.
